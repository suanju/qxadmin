...
执行任务过程中，如果发生跨模块、跨目录、数据库结构、RBAC、登录鉴权、后台核心页面或大量文件调整等大改动，必须在完成一个可验证阶段后自动进行 Git 暂存。

暂存时必须遵守：

- 先通过 `git status --short` 确认工作区状态
- 只暂存本次任务实际修改或新增的文件
- 不暂存用户原本已有、与本次任务无关的未提交改动
- 暂存后再次通过 `git status --short` 核对暂存范围
- 如果暂存范围无法与用户既有改动安全区分，必须先说明风险并询问用户
